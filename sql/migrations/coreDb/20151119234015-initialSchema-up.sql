...
CREATE TABLE IF NOT EXISTS `__CORE_DB_NAME__`.`agents` (
  ...
  UNIQUE KEY (`key`),
  UNIQUE KEY (`uuid`),
  CONSTRAINT `FK_agent_tenants` FOREIGN KEY (`tenant_uuid`) REFERENCES `__CORE_DB_NAME__`.`tenants`(`uuid`)
    ON DELETE CASCADE
)
  ENGINE = InnoDB
  DEFAULT CHARSET = utf8
...
CREATE TABLE IF NOT EXISTS `__CORE_DB_NAME__`.`tenant_connections` (
  ...
  `uuid`                     CHAR(36)            NOT NULL,
  `tenant_uuid`              CHAR(36)            NOT NULL,
  `tenant_organization_uuid` CHAR(36)                     DEFAULT NULL,
  `title`                    VARCHAR(40)         NOT NULL DEFAULT '',
  `description`              VARCHAR(1024)       NOT NULL DEFAULT '',
  ...
  CONSTRAINT `FK_tenant_connections_tenants` FOREIGN KEY (`tenant_uuid`) REFERENCES `__CORE_DB_NAME__`.`tenants`(`uuid`)
    ON DELETE CASCADE,

  CONSTRAINT `FK_tenant_connections_tenant_organizations` FOREIGN KEY (`tenant_organization_uuid`) REFERENCES `__CORE_DB_NAME__`.`tenant_organizations`(`uuid`)
    ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS `__CORE_DB_NAME__`.`tenant_organizations` (
  ...
  CONSTRAINT `FK_tenant_organization_tenants` FOREIGN KEY (`tenant_uuid`) REFERENCES `__CORE_DB_NAME__`.`tenants`(`uuid`)
    ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS `__CORE_DB_NAME__`.`tenant_members` (
  ...
  `uuid`            CHAR(36)         NOT NULL,
  `tenant_uuid`     CHAR(36)         NOT NULL,
  `cloud_user_uuid` CHAR(36)         NOT NULL,
  `reference_id`    VARCHAR(64)      NOT NULL DEFAULT '',
  `status`          INT(3) UNSIGNED  NOT NULL DEFAULT '1',
  `created`         TIMESTAMP        NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `timestamp`       TIMESTAMP        NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,

  CONSTRAINT `FK_tenant_members_tenants` FOREIGN KEY (`tenant_uuid`) REFERENCES `__CORE_DB_NAME__`.`tenants`(`uuid`)
    ON DELETE CASCADE,

  CONSTRAINT `FK_tenant_members_cloud_users` FOREIGN KEY (`cloud_user_uuid`) REFERENCES `__CORE_DB_NAME__`.`cloud_users`(`uuid`)
    ON DELETE CASCADE,

  PRIMARY KEY (`id`),
  UNIQUE KEY (`uuid`),
  UNIQUE KEY `tenant_member_reference_id`(`tenant_uuid`, `reference_id`)
)
  ENGINE = InnoDB
  DEFAULT CHARSET = utf8
  COLLATE = utf8_unicode_ci
...
CREATE TABLE IF NOT EXISTS `__CORE_DB_NAME__`.`tenant_access_roles` (
  ...
  CONSTRAINT `FK_tenant_access_roles_tenants` FOREIGN KEY (`tenant_uuid`) REFERENCES `__CORE_DB_NAME__`.`tenants`(`uuid`)
    ON DELETE CASCADE,

  CONSTRAINT `FK_tenant_access_roles_tenant_organizations` FOREIGN KEY (`tenant_organization_uuid`) REFERENCES `__CORE_DB_NAME__`.`tenant_organizations`(`uuid`)
    ON DELETE CASCADE,

  PRIMARY KEY (`id`),
  UNIQUE KEY (`uuid`),
  UNIQUE KEY `tenant_access_role_title`(`tenant_uuid`, `tenant_organization_uuid`, `title`)
)
  ENGINE = InnoDB
  DEFAULT CHARSET = utf8
  COLLATE = utf8_unicode_ci
...
CREATE TABLE IF NOT EXISTS `__CORE_DB_NAME__`.`tenant_access_role_assignments` (
  `id`                      INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
  `uuid`                    CHAR(36)         NOT NULL,
  `tenant_access_role_uuid` CHAR(36)         NOT NULL,
  `cloud_user_uuid`         CHAR(36)         NOT NULL,
  `status`                  INT(3) UNSIGNED  NOT NULL DEFAULT '1',
  `created`                 TIMESTAMP        NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `timestamp`               TIMESTAMP        NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,

  CONSTRAINT `FK_tenant_access_role_assignments_role_id` FOREIGN KEY (`tenant_access_role_uuid`) REFERENCES `__CORE_DB_NAME__`.`tenant_access_roles`(`uuid`)
    ON DELETE CASCADE,

  CONSTRAINT `FK_tenant_access_role_assignments_cloud_users` FOREIGN KEY (`cloud_user_uuid`) REFERENCES `__CORE_DB_NAME__`.`cloud_users`(`uuid`)
    ON DELETE CASCADE,

  PRIMARY KEY (`id`),
  UNIQUE KEY (`uuid`),
  UNIQUE KEY `cloud_user_role_assignment`(`cloud_user_uuid`, `tenant_access_role_uuid`)
)
  ENGINE = InnoDB
  DEFAULT CHARSET = utf8
  COLLATE = utf8_unicode_ci
...
CREATE TABLE IF NOT EXISTS `__CORE_DB_NAME__`.`tenant_access_resources` (
  ...
  CONSTRAINT `FK_tenant_access_resources_tenants` FOREIGN KEY (`tenant_uuid`) REFERENCES `__CORE_DB_NAME__`.`tenants`(`uuid`)
    ON DELETE CASCADE,

  CONSTRAINT `FK_tenant_access_resources_tenant_organizations` FOREIGN KEY (`tenant_organization_uuid`) REFERENCES `__CORE_DB_NAME__`.`tenant_organizations`(`uuid`)
    ON DELETE CASCADE,

  PRIMARY KEY (`id`),
  UNIQUE KEY (`uuid`),
  UNIQUE KEY `tenant_access_resource_key`(`key`),
  UNIQUE KEY `tenant_access_resource_title`(`tenant_uuid`, `tenant_organization_uuid`, `title`),
  UNIQUE KEY `tenant_access_resource_uri_method`(`tenant_uuid`, `tenant_organization_uuid`, `uri`, `method`)
...
CREATE TABLE IF NOT EXISTS `__CORE_DB_NAME__`.`tenant_access_permissions` (
  ...
  `uuid`                        CHAR(36)         NOT NULL,
  `tenant_access_role_uuid`     CHAR(36)         NOT NULL,
  `tenant_access_resource_uuid` CHAR(36)         NOT NULL,
  `status`                      INT(3) UNSIGNED  NOT NULL DEFAULT '1',
  `created`                     TIMESTAMP        NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `timestamp`                   TIMESTAMP        NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,

  CONSTRAINT `FK_tenant_access_permissions_role_id` FOREIGN KEY (`tenant_access_role_uuid`) REFERENCES `__CORE_DB_NAME__`.`tenant_access_roles`(`uuid`)
    ON DELETE CASCADE,

  CONSTRAINT `FK_tenant_access_permissions_resource_id` FOREIGN KEY (`tenant_access_resource_uuid`) REFERENCES `__CORE_DB_NAME__`.`tenant_access_resources`(`uuid`)
    ON DELETE CASCADE,

  PRIMARY KEY (`id`),
  UNIQUE KEY (`uuid`),
  UNIQUE KEY `tenant_access_role_resource`(`tenant_access_resource_uuid`, `tenant_access_role_uuid`)
)
  ENGINE = InnoDB
  DEFAULT CHARSET = utf8
  COLLATE = utf8_unicode_ci
