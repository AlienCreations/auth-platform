...
describe('updateAgent', () => {
  ...
  // TENANT_ID IN BODY
  it('throws an error when given a tenantId not belonging to any known tenant', done => {
    updateAgent(KNOWN_TEST_KEY, {
      tenantId : FAKE_UNKNOWN_TENANT_ID
    }).catch(err => {
  ...
  it('updates an agent tenantId when given a valid tenantId', done => {
    updateAgent(KNOWN_TEST_KEY, {
      tenantId : FAKE_UPDATE_TENANT_ID
    }).then(data => {
      expect(data.affectedRows).toBe(1);
      getAgentByKey(KNOWN_TEST_KEY)
        .then((agent) => {
          expect(R.prop('tenantId', agent)).toBe(FAKE_UPDATE_TENANT_ID);
          done();
        });
  ...
  it('does not update anything when no req body is provided', done => {
    updateAgent(KNOWN_TEST_KEY, undefined)
      .then(res => {
