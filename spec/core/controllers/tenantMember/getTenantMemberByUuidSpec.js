'use strict';

const R            = require('ramda'),
      path         = require('path'),
      config       = require('config'),
      commonMocks  = require('../../../_helpers/commonMocks'),
      CSVConverter = require('csvtojson').Converter,
      converter    = new CSVConverter({});

const getTenantMemberByUuid = require('../../../../server/core/controllers/api/tenantMember/getTenantMemberByUuid');

const COMMON_PRIVATE_FIELDS = R.path(['api', 'COMMON_PRIVATE_FIELDS'], config);

const FAKE_UNKNOWN_TENANT_MEMBER_UUID   = commonMocks.COMMON_UUID,
      FAKE_MALFORMED_TENANT_MEMBER_UUID = 'foo';

let KNOWN_TEST_TENANT_MEMBER_DATA,
    KNOWN_TEST_TENANT_MEMBER_UUID;

describe('tenantMemberCtrl.getTenantMemberByUuid', () => {
  beforeAll(done => {
    converter.fromFile(path.resolve(__dirname, '../../../../run/env/test/seedData/coreDb/tenantMembers.csv'), (err, data) => {
      KNOWN_TEST_TENANT_MEMBER_DATA = R.compose(R.omit(COMMON_PRIVATE_FIELDS), R.head, commonMocks.transformDbColsToJsProps)(data);
      KNOWN_TEST_TENANT_MEMBER_UUID = KNOWN_TEST_TENANT_MEMBER_DATA.uuid;
      done();
    });
  });

  it('returns tenantMemberData when looking for an tenantMember by uuid', done => {
    getTenantMemberByUuid(KNOWN_TEST_TENANT_MEMBER_UUID)
      .then(res => {
        expect(res.referenceId)
          .toEqual(KNOWN_TEST_TENANT_MEMBER_DATA.referenceId);
        done();
      })
      .catch(done.fail);
  });

  it('throws an error when looking for an tenantMember that does not exist', done => {
    getTenantMemberByUuid(FAKE_UNKNOWN_TENANT_MEMBER_UUID)
      .then(done.fail)
      .catch(err => {
        expect(commonMocks.isNoResultsErr(err)).toBe(true);
        done();
      });
  });

  it('throws an error when given a malformed uuid', done => {
    getTenantMemberByUuid(FAKE_MALFORMED_TENANT_MEMBER_UUID)
      .then(done.fail)
      .catch(err => {
        expect(commonMocks.isIllegalParamErr(err)).toBe(true);
        done();
      });
  });

  it('throws an error when looking for an tenantMember without an uuid', done => {
    getTenantMemberByUuid()
      .then(done.fail)
      .catch(err => {
        expect(commonMocks.isMissingParamErr(err)).toBe(true);
        done();
      });
  });

  it('throws an error when given null params', done => {
    getTenantMemberByUuid(null)
      .then(done.fail)
      .catch(err => {
        expect(commonMocks.isMissingParamErr(err)).toBe(true);
        done();
      });
  });
});
