...
describe('tenantCtrl.updateTenant', () => {
  beforeAll(done => {
    converter.fromFile(path.resolve(__dirname, '../../../../run/env/test/seedData/coreDb/tenants.csv'), (err, data) => {
      ...
      KNOWN_TEST_TENANT_UUID                  = R.prop('uuid', KNOWN_TEST_TENANT_DATA);
  ...
  it('updates an tenant when provided a uuid and new properties to update', done => {
    updateTenant(FAKE_TENANT_UPDATE_DATA, KNOWN_TEST_TENANT_UUID)
      .then(res => {
  ...
  it('throws an error when updating an tenant that does not exist', done => {
    updateTenant(FAKE_TENANT_UPDATE_DATA, FAKE_UNKNOWN_TENANT_UUID)
      .then(done.fail)
      .catch(err => {
  ...
  it('throws an error when updating with an existing domain', done => {
    updateTenant(FAKE_TENANT_UPDATE_DATA_EXISTING_DOMAIN, KNOWN_TEST_TENANT_UUID)
      .then(done.fail)
      .catch(err => {
