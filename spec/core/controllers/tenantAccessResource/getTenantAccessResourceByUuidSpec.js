'use strict';

const R            = require('ramda'),
      path         = require('path'),
      config       = require('config'),
      commonMocks  = require('../../../_helpers/commonMocks'),
      CSVConverter = require('csvtojson').Converter,
      converter    = new CSVConverter({});

const getTenantAccessResourceByUuid = require('../../../../server/core/controllers/api/tenantAccessResource/getTenantAccessResourceByUuid');

const COMMON_PRIVATE_FIELDS = R.path(['api', 'COMMON_PRIVATE_FIELDS'], config);

const FAKE_UNKNOWN_TENANT_ACCESS_RESOURCE_UUID   = commonMocks.COMMON_UUID,
      FAKE_MALFORMED_TENANT_ACCESS_RESOURCE_UUID = 'foo';

let KNOWN_TEST_TENANT_ACCESS_RESOURCE_DATA,
    KNOWN_TEST_TENANT_ACCESS_RESOURCE_UUID;

describe('tenantAccessResourceCtrl.getTenantAccessResourceByUuid', () => {
  beforeAll(done => {
    converter.fromFile(path.resolve(__dirname, '../../../../run/env/test/seedData/coreDb/tenantAccessResources.csv'), (err, data) => {
      KNOWN_TEST_TENANT_ACCESS_RESOURCE_DATA = R.compose(R.omit(COMMON_PRIVATE_FIELDS), R.head, commonMocks.transformDbColsToJsProps)(data);
      KNOWN_TEST_TENANT_ACCESS_RESOURCE_UUID = KNOWN_TEST_TENANT_ACCESS_RESOURCE_DATA.uuid;
      done();
    });
  });

  it('returns tenantAccessResourceData when looking for an tenantAccessResource by uuid', done => {
    getTenantAccessResourceByUuid(KNOWN_TEST_TENANT_ACCESS_RESOURCE_UUID)
      .then(res => {
        expect(res.key)
          .toEqual(KNOWN_TEST_TENANT_ACCESS_RESOURCE_DATA.key);
        done();
      })
      .catch(done.fail);
  });

  it('throws an error when looking for an tenantAccessResource that does not exist', done => {
    getTenantAccessResourceByUuid(FAKE_UNKNOWN_TENANT_ACCESS_RESOURCE_UUID)
      .then(done.fail)
      .catch(err => {
        expect(commonMocks.isNoResultsErr(err)).toBe(true);
        done();
      });
  });

  it('throws an error when given a malformed uuid', done => {
    getTenantAccessResourceByUuid(FAKE_MALFORMED_TENANT_ACCESS_RESOURCE_UUID)
      .then(done.fail)
      .catch(err => {
        expect(commonMocks.isIllegalParamErr(err)).toBe(true);
        done();
      });
  });

  it('throws an error when looking for an tenantAccessResource without an uuid', done => {
    getTenantAccessResourceByUuid()
      .then(done.fail)
      .catch(err => {
        expect(commonMocks.isMissingParamErr(err)).toBe(true);
        done();
      });
  });

  it('throws an error when given null params', done => {
    getTenantAccessResourceByUuid(null)
      .then(done.fail)
      .catch(err => {
        expect(commonMocks.isMissingParamErr(err)).toBe(true);
        done();
      });
  });
});
